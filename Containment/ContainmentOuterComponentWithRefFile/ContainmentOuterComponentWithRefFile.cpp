#include <windows.h>
#include "ContainmentOuterComponentWithRefFile.h"
...
BOOL WINAPI DllMain(HINSTANCE hDll, DWORD dwReason, LPVOID Reserved)
{
	...
	case DLL_PROCESS_ATTACH:
		MessageBox(NULL, TEXT("DLL_PROCESS_ATTACH_OUTTER"), TEXT("Message"), MB_OK);
		break;
	case DLL_PROCESS_DETACH:
		MessageBox(NULL, TEXT("DLL_PROCESS_DETACH_OUTER"), TEXT("Message"), MB_OK);
		break;
	}
	return(TRUE);
}
...
{
	MessageBox(NULL, TEXT("CoClass QueryInterface Inner"), TEXT("Message"), MB_OK);
...
{
	MessageBox(NULL, TEXT("CoClass_InitializeInnerComponent_Outer"), TEXT("Message"), MB_OK);
	HRESULT hr;
	hr = CoCreateInstance(CLSID_MultiplicationDivision, NULL, CLSCTX_INPROC_SERVER, IID_IMultiplication, (void **)&m_pIMultiplication);

	if (FAILED(hr))
	{
		MessageBox(NULL, TEXT("IMultiplication Interface Can Not Br Obtained From Inner Component"), TEXT("ERROR"), MB_OK);
		return(E_FAIL);
	}
...
{
	MessageBox(NULL, TEXT("ClassFactory_QueryInterface_Outer"), TEXT("Message"), MB_OK);
	if (riid == IID_IUnknown)
...
{
	MessageBox(NULL, TEXT("ClassFactory_CreateInstance_Outer"), TEXT("Message"), MB_OK);
